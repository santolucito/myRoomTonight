...
homePage =
    ok $ embedInTemplate "home page" $ do
           H.h1 "Hello!"
...
queryParams =
    do mFoo <- optional $ lookText "foo"
       ok $ embedInTemplate "query params" $ do
...
upload =
     ...
     uploadForm =
         do method GET
            ok $ embedInTemplate "upload form" $ do
     ...
     handleUpload =
         do (tmpFile, uploadName, contentType) <- lookFile "file_upload"
            ok $ embedInTemplate "file uploaded" $ do
